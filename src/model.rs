...
fn model_run(model: &TractModel, input: &Float32Array) -> Result<InferResult, JsValue> {
    ...
    let output = model.run(tvec![input_tensor.into()])
    ...
    // スコアをインデックスと共に保存
    let mut scores: InferResult = values.iter()
        .enumerate()
        .map(|(i, &score)| (i, score))
        .collect();
...
mod tests {
    ...
    fn test_infer_top5() {
        ...
        for i in 0..top5.len()-1 {
            assert!(top5[i].1 >= top5[i+1].1, "トップ5のスコアが降順ソートされていない");
        }
    }
